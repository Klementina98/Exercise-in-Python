...
def get_words(doc):
    ...
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20 and word.lower() not in najpojavuvaniZborovi:
...
def calculate_document_frequencies(documents):
    ...
    documents_words = []
    for doc_text in documents:
        words = get_words2(doc_text) #zoshto ovde ne ja smenivme vo get_words  bidejki ja koristime koga gi naogjame site zborovi pa od ne posle gi vadime 100najpojauvani
...
#kako da ja definiram listata nadvor od main



if __name__ == '__main__':
    text = input()
    #da gi najdam 100te najpojavuvani zborovi vo mnozestvoto
    documents=[]
    for doc in train_data:
        documents.append(doc[0])
    df=calculate_document_frequencies(documents)
    #da go sortirame alfabetski, pa spored br. na pojavuvanja
    df=dict(sorted(df.items(), key=lambda x:x[0], reverse=False))
    #sega spored broj na pojavuvanja
    lista=sorted(df.items(), key=lambda x: x[1], reverse=True)
    #da gi zememe samo zboroviite
    najpojavuvaniZborovi=[]
    for i in lista[:100]:
        najpojavuvaniZborovi.append(i[0])
    N=len(documents)
    vocab=get_vocabulary(documents)

    slichniKosinusno=rank_documents(text,documents,cosine)
    slichniPirsonova=rank_documents(text,documents,pearson)

    for i in range(5):
        print("Rang po slicnost:" + str(i) + " Cosine-Document #" + str(slichniKosinusno[i][1]) + " Pearson-Document #" + str(slichniPirsonova[i][1]))
